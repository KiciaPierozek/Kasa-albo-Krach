...
void Silnik::wBanku() {
	bool wyjscie = false;
	bool wKredycie = peKO.getCzyKredyt();
	long double kwota;
	while (!wyjscie) {
		wyswietlacz.wBanku();
		std::cin >> wybor;
		...
		case 2:
			...
			wyswietlacz.splacanieKredytu(peKO);
			std::cin >> kwota;
			if (kwota < 0 || kwota > peKO.getKredyt().getWartosc()) {
				wyswietlacz.zlaKwotaKredytu();
				break;
			}
			peKO.getKredyt().splac(kwota);
			wyswietlacz.infoSplacenie(peKO, kwota);
			break;
		case 3:
